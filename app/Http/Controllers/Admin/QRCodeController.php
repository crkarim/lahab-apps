...
class QRCodeController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $request->validate([
            ...
            'logo' => 'image|max:'. $this->maxImageSizeKB .'|mimes:' . implode(',', array_column(IMAGEEXTENSION, 'key')),
        ]);

        $currentLogo = Helpers::get_business_settings('qr_code');
        $logo = $request->file('logo') ? Helpers::update('qrcode/', $currentLogo['logo']??'',APPLICATION_IMAGE_FORMAT, $request->file('logo')) : $currentLogo['logo']??'';
