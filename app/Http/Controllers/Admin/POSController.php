...
class POSController extends Controller
{
    ...
    public function update_discount(Request $request): RedirectResponse
    {
        ...
        foreach (session('cart') as $cartItem) {
            if (isset($cartItem['price'])) {
                $totalAmount += ($cartItem['price'] - $cartItem['discount']) * $cartItem['quantity'];
            }
        }
        ...
        if ($request->type == 'amount' && $request->discount > $totalAmount) {
            Toastr::error(translate('Extra_discount_can_not_be_more_than ') .$totalAmount);
            return back();
        }
    ...
    public function couponApply(Request $request): RedirectResponse
    {
        ...
        foreach (session('cart') as $cart) {
            if (isset($cart['price'])) {
                $totalAmount += ($cart['price'] - $cart['discount']) * $cart['quantity'];
            }
        }
        ...
        $couponData = applyPOSCoupon(code: $request['coupon_code'], amount: $totalAmount, userId: $sessionCustomerId, isGuest: $isGuest);

        $couponDiscount = $couponData['coupon_discount_amount'];
        $totalAfterCoupon = $totalAmount - $couponDiscount;

        $cart = session()->get('cart', []);

        $cart['coupon_code'] = $couponData['code'];
        $cart['coupon_discount'] = $couponDiscount;

        session()->put('cart', $cart);

        if ($couponData['code'] != null) {
            Toastr::success(translate('coupon_apply_successfully'));
        } else {
            Toastr::error(translate('coupon_not_apply_successfully'));
        }
    ...
    public function place_order(Request $request): RedirectResponse
    {
        ...
        if (isset($cart['coupon_code']) &&  $cart['coupon_code'] != null){
            $couponData = applyPOSCoupon(code: $cart['coupon_code'], amount: $total_price, userId: $sessionCustomerId, isGuest: $isGuest);
            $couponCode = $couponData['code'];
            $couponDiscountAmount = $couponData['coupon_discount_amount'];
            $total_price -=$couponDiscountAmount;
        }
        ...
        if (isset($cart['extra_discount'])) {
            if ($total_price_for_discount_validation > 0){

                if($cart['extra_discount_type'] == 'percent' && $cart['extra_discount'] > 0){
                    $extra_discount = ($total_price * $cart['extra_discount']) / 100;
                }else{
                    $extra_discount = min($total_price, $cart['extra_discount']);
                }
        ...
        try {
            ...
            $order->order_amount = Helpers::set_price($total_price + $total_tax_amount + $total_addon_tax);
            $order->delivery_charge = $delivery_charge;
            $order->coupon_code = $couponCode;
            $order->coupon_discount_amount = Helpers::set_price($couponDiscountAmount);
    ...
    {
        if ($request['key'] == 'customer_id' && $request['value'] == 0){
            $request['value'] = null;
        }
        session()->put($request['key'], $request['value']);

        if ($request['key'] == 'customer_id' && session()->has('cart')){
            $cart = session()->get('cart');
    ...
    public function updateCustomer(Request $request): RedirectResponse
    {
        ...
        $user = $this->user->findOrFail($request->customer_id);
    ...
    public function couponList(Request $request): JsonResponse
    {
        ...
        foreach ($coupons as $coupon) {
            if (isPOSCouponAvailable($coupon, $amount, $sessionCustomerId, $isGuest)) {
                $availableCoupons[] = $coupon;
            }
        }

        $html = view('admin-views.pos.coupon_items', [
    ...
    {
        $value = $request->value == 0 ? null : $request->value;
        ...
        if (!empty($cart)) {
            unset($cart['coupon_code']);
            unset($cart['coupon_discount']);
            unset($cart['extra_discount_type']);
            unset($cart['extra_discount']);

            session()->put('cart', $cart);
        }

        return response()->json([
            'view' => view('admin-views.pos._cart')->render(),
