...
class CategoryController extends Controller
{
    ...
    function store(Request $request): RedirectResponse
    {
        ...
        $request->validate([
            'name' => 'required',
            'image' => 'sometimes|image|max:'. $this->maxImageSizeKB .'|mimes:' . implode(',', array_column(IMAGEEXTENSION, 'key')),
            'banner_image' => 'sometimes|image|max:'. $this->maxImageSizeKB .'|mimes:' . implode(',', array_column(IMAGEEXTENSION, 'key')),
        ]);
        ...
        if (!empty($request->file('image'))) {
            $imageName = Helpers::upload('category/', APPLICATION_IMAGE_FORMAT, $request->file('image'));
        } else {
            $imageName = 'def.png';
        }
        if (!empty($request->file('banner_image'))) {
            $bannerImageName = Helpers::upload('category/banner/', APPLICATION_IMAGE_FORMAT, $request->file('banner_image'));
        } else {
