...
class BusinessSettingsController extends Controller
{
    ...
    public function restaurantSetup(Request $request): RedirectResponse
    {
        ...
        $this->InsertOrUpdateBusinessData(['key' => 'logo'], [
            'value' => $request->has('logo') ? Helpers::update('restaurant/', $currentLogo->value, APPLICATION_IMAGE_FORMAT, $request->file('logo')) : $currentLogo->value
        ]);
    ...
    {
        $request->validate([
            'service_file' => [  'nullable', 'file', 'mimes:p8', 'max:' . $this->maxImageSizeKB],
        ]);
