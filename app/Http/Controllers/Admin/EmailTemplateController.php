...
class EmailTemplateController extends Controller
{
    ...
    public function updateEmailIndex(Request $request, $type, $tab): RedirectResponse
    {
        ...
        $template->background_image = $request->has('background_image') ? Helpers::update('email_template/', $template->background_image, APPLICATION_IMAGE_FORMAT, $request->file('background_image')) : $template->background_image;
        $template->image = $request->has('image') ? Helpers::update('email_template/', $template->image, APPLICATION_IMAGE_FORMAT, $request->file('image')) : $template->image;
        $template->logo = $request->has('logo') ? Helpers::update('email_template/', $template->logo, APPLICATION_IMAGE_FORMAT, $request->file('logo')) : $template->logo;
        $template->icon = $request->has('icon') ? Helpers::update('email_template/', $template->icon, APPLICATION_IMAGE_FORMAT, $request->file('icon')) : $template->icon;
