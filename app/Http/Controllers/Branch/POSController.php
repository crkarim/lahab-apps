...
class POSController extends Controller
{
    ...
    public function placeOrder(Request $request): RedirectResponse
    {
        ...
        if (isset($cart['coupon_code']) &&  $cart['coupon_code'] != null){
            $couponData = applyPOSCoupon(code: $cart['coupon_code'], amount: $totalPrice, userId: $sessionCustomerId, isGuest: $isGuest);
        ...
        if (isset($cart['extra_discount'])) {
            if ($totalPriceForDiscountValidation > 0){

                if($cart['extra_discount_type'] == 'percent' && $cart['extra_discount'] > 0){
                    $extraDiscount = ($totalPrice * $cart['extra_discount']) / 100;
                }else{
                    $extraDiscount = min($totalPrice, $cart['extra_discount']);
                }
        ...
        try {
            ...
            $order->order_amount = Helpers::set_price($totalPrice + $totalTaxAmount + $totalAddonTax);
    ...
    public function couponList(Request $request): JsonResponse
    {
        ...
        $html = view('branch-views.pos.coupon_items', [
    ...
    public function changeCustomer(Request $request): JsonResponse
    {
        ...
        return response()->json([
            'view' => view('branch-views.pos._cart')->render(),
