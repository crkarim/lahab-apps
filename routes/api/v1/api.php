...
Route::group(['namespace' => 'Api\V1', 'middleware' => 'localization'], function () {
    ...
    Route::group(['prefix' => 'auth', 'namespace' => 'Auth'], function () {
        Route::post('registration', [CustomerAuthController::class, 'registration'])->name('api.v1.auth.registration');
        Route::post('login', [CustomerAuthController::class, 'login'])->name('api.v1.auth.login');
        ...
        Route::post('forgot-password', [PasswordResetController::class, 'passwordResetRequest'])->name('api.v1.password.request');
        Route::post('verify-token', [PasswordResetController::class, 'verifyToken']);
        Route::put('reset-password', [PasswordResetController::class, 'resetPasswordSubmit'])->name('api.v1.password.update');

        Route::group(['prefix' => 'delivery-man'], function () {
            Route::post('register', [DeliveryManLoginController::class, 'registration'])->name('api.v1.delivery-man.register');
            Route::post('login', [DeliveryManLoginController::class, 'login'])->name('api.v1.delivery-man.login');
        });
    ...
    Route::group(['prefix' => 'products', 'middleware' => 'branch_adder'], function () {
        ...
        Route::post('reviews/submit', [ProductController::class, 'submitProductReview'])->name('api.v1.product.review.submit')->middleware('auth:api');
    ...
    Route::group(['prefix' => 'customer', 'middleware' => ['auth:api', 'is_active']], function () {
        ...
        Route::group(['prefix' => 'order'], function () {
            ...
            Route::post('place', [OrderController::class, 'placeOrder'])->withoutMiddleware(['auth:api', 'is_active'])->name('api.v1.order.place');
    ...
    Route::post('subscribe-newsletter', [CustomerController::class, 'subscribeNewsletter'])->name('api.v1.subscribe.newsletter');
    ...
    Route::group(['prefix' => 'table', 'middleware' => 'app_activate:' . APPS['table_app']['software_id']], function () {
        ...
        Route::post('order/place', [TableController::class, 'placeOrder'])->name('api.v1.table.order.place');
